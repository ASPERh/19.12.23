...
public:
    Smartphone() : brand(""), model(""), screenSize(0.0), batteryLife(0), storageCapacity(0) {}

    Smartphone(const string& brand, const string& model, double screenSize, int batteryLife, int storageCapacity)
        : brand(brand), model(model), screenSize(screenSize), batteryLife(batteryLife), storageCapacity(storageCapacity) {
    }
...
public:
    Cat() : name(""), age(0), color(""), breed(""), isHungry(false) {}

    Cat(const string& name, int age, const string& color, const string& breed, bool isHungry)
        : name(name), age(age), color(color), breed(breed), isHungry(isHungry) {
    }
...
public:
    Pen() : color(""), brand(""), isBallpoint(false), inkLevel(0.0), isCapPresent(false) {}

    Pen(const string& color, const string& brand, bool isBallpoint, double inkLevel, bool isCapPresent)
        : color(color), brand(brand), isBallpoint(isBallpoint), inkLevel(inkLevel), isCapPresent(isCapPresent) {
    }
...
public:
    RemoteControl() : brand(""), numOfButtons(0), hasBattery(false), isProgrammable(false), compatibleDevices("") {}

    RemoteControl(const string& brand, int numOfButtons, bool hasBattery, bool isProgrammable, const string& compatibleDevices)
        : brand(brand), numOfButtons(numOfButtons), hasBattery(hasBattery), isProgrammable(isProgrammable), compatibleDevices(compatibleDevices) {
    }
...
public:
    Coin() : value(0), countryOfOrigin(""), yearOfMint(0), material(""), isRare(false) {}

    Coin(int value, const string& countryOfOrigin, int yearOfMint, const string& material, bool isRare)
        : value(value), countryOfOrigin(countryOfOrigin), yearOfMint(yearOfMint), material(material), isRare(isRare) {
    }
